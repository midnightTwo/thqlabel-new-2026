export default function Dashboard() {
  return (
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <aside className="hidden md:block md:w-64 md:fixed md:left-0 md:top-0 md:h-screen border-r border-white/5 pt-24 px-6 glass-morphism-sidebar">
      ...
      <main className="w-full md:ml-64 pt-24 sm:pt-28 md:pt-32 px-4 sm:px-6 md:px-12">
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-8 mb-12 sm:mb-16">
          <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full border-2 border-neon-blue p-1 shadow-[0_0_20px_rgba(0,243,255,0.2)] glass-morphism">
          ...
          <div className="text-center sm:text-left">
            ...
            <button className="px-4 sm:px-6 py-1.5 sm:py-2 glass-morphism-button border border-neon-blue text-neon-blue rounded-full text-[9px] sm:text-[10px] font-bold uppercase hover:bg-neon-blue hover:text-black transition">Edit Profile</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-12">
          {['Monthly Listeners', 'Total Streams', 'Revenue'].map((label) => (
            <div key={label} className="glass-morphism-card p-4 sm:p-6 rounded-2xl border border-white/5 hover:border-purple-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/10">
