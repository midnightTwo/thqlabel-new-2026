  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        ...
        <button 
          onClick={onCancel} 
          className={`px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm rounded-lg transition flex items-center gap-1 sm:gap-1.5 border touch-manipulation ${
            isLight 
              ? 'text-gray-600 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 border-gray-300 hover:border-gray-400' 
              : 'text-zinc-400 hover:text-white bg-white/5 hover:bg-white/10 border-white/10 hover:border-white/20'
          }`}
        >
      ...
      <div>
        ...
        <input 
          ...
          className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${
            isLight 
              ? `bg-white border-gray-300 text-gray-900 placeholder:text-gray-400 ${isSingleRelease ? 'opacity-60 cursor-not-allowed ring-2 ring-emerald-500/20' : 'hover:border-gray-400 focus:border-[#6050ba] focus:ring-2 focus:ring-[#6050ba]/20'}`
              : `bg-gradient-to-br from-white/[0.07] to-white/[0.03] border-white/10 text-white placeholder:text-zinc-600 ${isSingleRelease ? 'opacity-60 cursor-not-allowed ring-2 ring-emerald-500/20' : 'hover:border-white/20 focus:border-[#6050ba] focus:ring-2 focus:ring-[#6050ba]/20'}`
          }`}
        />
      </div>
      ...
      <div>
        <label className={`text-sm mb-2 flex items-center gap-2 ${isLight ? 'text-gray-600' : 'text-zinc-400'}`}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-purple-400/70">
            <path d="M4 7h16" />
            <path d="M4 12h10" />
            <path d="M4 17h16" />
        ...
        <input
          ...
          className={`w-full px-4 py-3 rounded-xl border outline-none transition-all font-mono text-sm ${
            isLight
      ...
      {/* Instrumental Toggle */}
      <label className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition group ${
        isLight 
          ? 'bg-blue-50 border-blue-200 hover:bg-blue-100' 
          : 'bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border-blue-500/20 hover:bg-blue-500/15'
      }`}>
        <div className="relative">
          ...
          />
          <div className={`w-11 h-6 rounded-full peer peer-checked:bg-blue-500 transition-colors ${isLight ? 'bg-gray-300' : 'bg-zinc-700'}`}></div>
          <div className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-5"></div>
        </div>
        <div>
          <div className={`text-sm font-bold ${isLight ? 'text-gray-900' : 'text-white'}`}>Инструментал</div>
      ...
      {!trackIsInstrumental && (
        <div>
          <label className={`text-sm mb-2 flex items-center gap-2 ${isLight ? 'text-gray-600' : 'text-zinc-400'}`}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-violet-400/70">
              <path d="M5 8l6 6"/>
          ...
          <select value={trackLanguage} onChange={(e) => setTrackLanguage(e.target.value)} className={`w-full px-4 py-3 rounded-xl border outline-none appearance-none cursor-pointer ${
            isLight 
              ? 'bg-white border-gray-300 text-gray-900' 
              : 'bg-gradient-to-br from-white/[0.07] to-white/[0.03] border-white/10 text-white'
          }`}>
      ...
      <div className="space-y-4">
          ...
          <input 
            ...
            className={`w-full px-4 py-3 rounded-xl border outline-none ${
              isLight 
                ...
                : 'bg-gradient-to-br from-white/[0.07] to-white/[0.03] border-white/10 text-white placeholder:text-zinc-600'
            }`} 
          />
        </div>
        ...
        {/* Producers */}
        <div>
          <label className={`text-xs sm:text-sm mb-2 flex items-center gap-2 ${isLight ? 'text-gray-600' : 'text-zinc-400'}`}>
            ...
            Продюсеры {trackProducers && trackProducers.filter(p => p.trim()).length > 0 && (
              <span className={`text-xs ${isLight ? 'text-gray-500' : 'text-zinc-600'}`}>({trackProducers.filter(p => p.trim()).length}/10)</span>
            )}
          </label>
          <div className="flex gap-1.5 sm:gap-2">
            <input 
              ...
              className={`flex-1 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl border outline-none disabled:opacity-50 text-sm sm:text-base ${
                isLight 
                  ? 'bg-white border-gray-300 text-gray-900 placeholder:text-gray-400' 
                  : 'bg-gradient-to-br from-white/[0.07] to-white/[0.03] border-white/10 text-white placeholder:text-zinc-600'
              }`} 
            ...
            <button
              ...
              disabled={trackProducers && trackProducers.filter(p => p.trim()).length >= 10}
              className={`px-3 sm:px-4 py-2.5 sm:py-3 border rounded-xl transition disabled:opacity-30 min-w-[44px] touch-manipulation flex items-center justify-center ${
                isLight 
                  ? 'bg-blue-50 hover:bg-blue-100 border-blue-200 text-blue-400 hover:text-blue-600' 
                  : 'bg-blue-500/10 hover:bg-blue-500/20 border-blue-500/20 text-blue-400/50 hover:text-blue-300'
              }`}
            >
          ...
          {trackProducers && trackProducers.filter(p => p.trim()).length > 0 && (
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-2">
              {trackProducers.filter(p => p.trim()).map((producer, idx) => (
                <div key={idx} className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm flex items-center gap-1.5 sm:gap-2 ${
                  isLight 
                    ? 'bg-blue-100 border border-blue-200 text-blue-700' 
                    : 'bg-blue-500/10 border border-blue-500/20 text-blue-300'
                }`}>
                  <span className="truncate max-w-[120px] sm:max-w-none">prod. {producer}</span>
                  <button type="button" onClick={() => setTrackProducers?.(trackProducers.filter((_, i) => i !== idx))}
                    className={`transition touch-manipulation min-w-[20px] min-h-[20px] flex items-center justify-center ${
                      isLight ? 'text-blue-400 hover:text-red-500' : 'text-blue-400/50 hover:text-red-400'
                    }`}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5">
        ...
        <div>
          <label className={`text-xs sm:text-sm mb-2 flex items-center gap-2 ${isLight ? 'text-gray-600' : 'text-zinc-400'}`}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-pink-400/70">
            ...
            Featuring артисты {trackFeaturing && trackFeaturing.filter(f => f.trim()).length > 0 && (
              <span className={`text-xs ${isLight ? 'text-gray-500' : 'text-zinc-600'}`}>({trackFeaturing.filter(f => f.trim()).length}/10)</span>
            )}
            <span className={`text-[10px] sm:text-xs ${isLight ? 'text-gray-500' : 'text-zinc-600'}`}>(feat. в названии)</span>
          </label>
          <div className="flex gap-1.5 sm:gap-2">
            <input 
              id="featuring-input"
            ...
            <button
              ...
              disabled={(trackFeaturing && trackFeaturing.filter(f => f.trim()).length >= 10)}
              className={`px-3 sm:px-4 py-2.5 sm:py-3 border rounded-xl transition min-w-[44px] touch-manipulation flex items-center justify-center ${
                isLight 
                  ? 'bg-pink-50 hover:bg-pink-100 border-pink-200 text-pink-400 hover:text-pink-600' 
                  : 'bg-pink-500/10 hover:bg-pink-500/20 border-pink-500/20 text-pink-400/50 hover:text-pink-300'
              }`}
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5">
                <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-2">
              {trackFeaturing.filter(f => f.trim()).map((feat, idx) => (
                <div key={idx} className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm flex items-center gap-1.5 sm:gap-2 ${
                  ...
                  <span className="truncate max-w-[120px] sm:max-w-none">feat. {feat}</span>
                  <button type="button" onClick={() => setTrackFeaturing?.(trackFeaturing.filter((_, i) => i !== idx))}
                    className={`transition touch-manipulation min-w-[20px] min-h-[20px] flex items-center justify-center ${
                      isLight ? 'text-pink-400 hover:text-red-500' : 'text-pink-400/50 hover:text-red-400'
                    }`}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5">
                      <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
                    </svg>
      ...
      <label className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition group ${
        isLight 
          ...
          : 'bg-gradient-to-r from-red-500/10 to-orange-500/5 border-red-500/15 hover:bg-red-500/15'
      }`}>
        <div className="relative">
          <input 
            type="checkbox" 
            ...
            className="sr-only peer" 
          />
          <div className={`w-11 h-6 rounded-full peer peer-checked:bg-red-500/60 transition-colors ${isLight ? 'bg-gray-300' : 'bg-zinc-700'}`}></div>
        ...
        <div className="flex items-center gap-2">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-red-400/70">
            <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/>
            <line x1="12" y1="9" x2="12" y2="13"/>
            <line x1="12" y1="17" x2="12.01" y2="17"/>
          </svg>
          <div>
            <div className={`text-sm font-bold ${isLight ? 'text-gray-900' : 'text-white'}`}>Explicit Content</div>
      ...
      {!trackIsInstrumental && (
        <div>
          <label className={`text-sm mb-2 flex items-center gap-2 ${isLight ? 'text-gray-600' : 'text-zinc-400'}`}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-indigo-400/70">
          ...
          <textarea value={trackLyrics} onChange={(e) => setTrackLyrics(e.target.value)} placeholder="Введите текст..." rows={8} 
            className={`w-full px-4 py-3 rounded-xl border outline-none resize-none overflow-y-auto ${
              isLight 
                ? 'bg-white border-gray-300 text-gray-900 placeholder:text-gray-400' 
                : 'bg-gradient-to-br from-white/[0.07] to-white/[0.03] border-white/10 text-white placeholder:text-zinc-600'
            }`} 
      ...
      <div className={`flex flex-col sm:flex-row gap-2 sm:gap-3 pt-4 border-t ${isLight ? 'border-gray-200' : 'border-white/10'}`}>
        ...
        <button onClick={onCancel} className={`px-4 sm:px-6 py-3 rounded-xl font-bold transition touch-manipulation text-sm sm:text-base ${
          isLight 
